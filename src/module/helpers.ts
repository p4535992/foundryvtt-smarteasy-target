...
export function drawBetterTarget(token, fillColor, p, aw, h, hh, w, hw, ah) {
  ...
  token.target
    ...
    .lineStyle(stroke + 2, 0x000000)
    .drawCircle(hmid, vmid, size / 2 - padding - stroke / 2)
    .endFill()
    .beginFill(0x000000, 0.0)
    .lineStyle(stroke, fillColor)
    .drawCircle(hmid, vmid, size / 2 - padding - stroke / 2)
    .endFill()
    .beginFill(fillColor, 1.0)
    .lineStyle(1, 0x000000)
    .drawRoundedRect(
      hmid - stroke / 2,
      vmid - stroke - padding / 2 - crossLen,
      stroke,
      crossLen,
      (stroke / 2) * smoothGraphicsHack,
    )
    .drawRoundedRect(
      hmid - stroke / 2,
      vmid + padding * 1.5 - stroke,
      stroke,
      crossLen,
      (stroke / 2) * smoothGraphicsHack,
    )
    .drawRoundedRect(
      hmid - stroke - padding / 2 - crossLen,
      vmid - stroke / 2,
      crossLen,
      stroke,
      (stroke / 2) * smoothGraphicsHack,
